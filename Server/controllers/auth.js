...
exports.facebookLogin= (req,res)=> {
    ...
    const {userID,accessToken,email} = req.body
    console.log('userId from client',userID)

    // const url = `https://graph.facebook.com/v2.11/${userID}/?fields = id,name,email&accessToken=${accesstToken}`
    ...
    return (
        ...
        .then(response => {
            const {name} = response
            console.log(`My email is ${email} and name is ${name}`)
            User.findOne({email}).exec((err,user)=> {
                if (user) {
                    const token = jwt.sign({ _id: user._id }, process.env.JWT_SECRET, { expiresIn: '7d' });
                    const { _id, email, name, role } = user;
